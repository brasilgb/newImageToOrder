    ...
    <>
      {props.data &&
        <View className='mx-4 pb-8'>
          <Text className="text-xl font-bold text-megb-blue-primary my-2 uppercase">{props.data[0]?.customer.name}</Text>
          {props?.data?.map((order: any, idx: number) => (
            <View key={idx} className="flex-row items-center justify-start mt-2 gap-2 bg-gray-50 p-3 rounded-md border border-white shadow-md shadow-gray-700">
              ...
              <Text className="flex-1 text-lg font-semibold text-gray-500">{order.equipment.equipment} - {order.model}</Text>
